    ...
    match e with
        ...
        | Index (name, ei, t, pos) ->
            ...
            *)
            ...
            let newName =
                match SymTab.lookup name vtable with
                | Some (VarProp vname) -> vname
                | _ -> name
            Index (newName, ei', t, pos)
        | Let (Dec (name, ed, decpos), body, pos) ->
            ...
            match ed' with
                | Var (vname, _) ->
                    ...
                    *)
                    // Check for shadowing: if `name` already in vtable, do NOT record.
                    let newVtable =
                        if SymTab.lookup name vtable |> Option.isSome then vtable
                        else SymTab.bind name (VarProp vname) vtable
                    copyConstPropFoldExp newVtable body
                | Constant (v, _) ->
                    (* TODO project task 3:
                    ...
                    *)
                    let newVtable =
                        if SymTab.lookup name vtable |> Option.isSome then vtable
                        else SymTab.bind name (ConstProp v) vtable
                    copyConstPropFoldExp newVtable body
                | Let (Dec (innerName, innerDef, innerPos), innerBody, _) ->
                    (* TODO project task 3:
                    ...
                    *)
                    // Flatten only if innerName is not already bound in vtable (avoid shadowing issues)
                    if SymTab.lookup innerName vtable |> Option.isSome then
                        // Shadowing – skip flattening, just optimize normally
                        let innerDef' = copyConstPropFoldExp vtable innerDef
                        let innerBody' = copyConstPropFoldExp vtable innerBody
                        Let (Dec (innerName, innerDef', innerPos),
                            Let (Dec (name, innerBody', decpos), body, pos),
                            innerPos)
                    else
                        let innerDef' = copyConstPropFoldExp vtable innerDef
                        let innerBody' = copyConstPropFoldExp vtable innerBody
                        let newBody = Let (Dec (name, innerBody', decpos), body, pos)
                        Let (Dec (innerName, innerDef', innerPos), newBody, innerPos)
        ...
        | And (e1, e2, pos) ->
            ...
            let e2' = copyConstPropFoldExp vtable e2
            match (e1', e2') with
                | (Constant (BoolVal x, _), Constant (BoolVal y, _)) -> Constant (BoolVal (x && y), pos)
