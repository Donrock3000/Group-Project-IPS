...
let rec evalExp (e : UntypedExp, vtab : VarTable, ftab : FunTable) : Value =
  ...
  *)
  ...
  | Divide(e1, e2, pos) ->
      ...
      | (IntVal n1, IntVal n2) ->
          if n2 = 0 then
              raise (MyError("Division by zero", pos))
          else
              IntVal (n1 / n2)
      | (IntVal _, _) -> reportWrongType "right operand of /" Int res2 (expPos e2)
      | (_, _) -> reportWrongType "left operand of /" Int res1 (expPos e1)
  | And(e1, e2, pos) ->
    match evalExp(e1, vtab, ftab) with
      ...
      | v -> reportWrongType "left operand of &&" Bool v pos
  | Or(e1, e2, pos) ->
    match evalExp(e1, vtab, ftab) with
  ...
  | Not(e, pos) ->
    ...
    match res with
      ...
      | _ -> reportWrongType "not argument" Bool res pos
  | Negate(e, pos) ->
      let res = evalExp(e, vtab, ftab)
      match res with
      | IntVal n -> IntVal (-n)
      | _ -> reportWrongType "negation argument" Int res pos
  | Equal(e1, e2, pos) ->
  ...
  (* Project task 2: `replicate(n, a)`
       ...
       - build an array with `n` copies of the value of `a`. *)
  ...
  (* Project task 2: `filter(p, arr)`
       - evaluate `arr` and check it is an array,
       - keep only the elements for which `p` returns `true`
         (checking the predicate really returns a boolean). *)
          ...
          | ArrayVal (lst, tp1) ->
               let pred x =
                   match evalFunArg (farg, vtab, ftab, pos, [x]) with
                     | BoolVal b -> b
                     | other -> reportWrongType "predicate result in filter" Bool other pos
               ArrayVal (List.filter pred lst, tp1)
          | otherwise -> reportNonArray "2nd argument of \"filter\"" arr pos

  (* Project task 2: `scan(f, ne, arr)`
       Like reduce, but produces an array of the same length as `arr`,
       holding all the intermediate accumulator values. *)
               ...
               let (_, rev_res) =
                   List.fold
                     (fun (acc, res) x ->
                         ...
                         (acc', acc' :: res))
                     (nel, []) lst
               ArrayVal (List.rev rev_res, farg_ret_type)
