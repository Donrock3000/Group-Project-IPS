...
let rec compileExp  (e      : TypedExp)
  ...
  *)
  ...
  | Not (e, pos) ->
    let t1 = newReg "not_tmp"
    let code1 = compileExp e vtable t1
    let lab_false = newLab "not_false"
    code1 @
        [ LI (place, 0)
        ; BNE (t1, Rzero, lab_false)
        ; LI (place, 1)
        ; LABEL lab_false ]

  | Negate (e, pos) ->
    let t1 = newReg "neg_tmp"
    let code1 = compileExp e vtable t1
  ...
  *)
  | And (e1, e2, pos) ->
    let t1 = newReg "and_tmp"
    let code1 = compileExp e1 vtable t1
    let lab_false = newLab "and_false"
    let lab_end = newLab "and_end"
    let t2 = newReg "and_tmp2"
    let code2 = compileExp e2 vtable t2
    code1 @
        [ BEQ (t1, Rzero, lab_false) ]
        @ code2 @
        [ LI (place, 1)
        ; J lab_end
        ; LABEL lab_false
  ...
  | Or (e1, e2, pos) ->
    let t1 = newReg "or_tmp"
    let code1 = compileExp e1 vtable t1
    let lab_true = newLab "or_true"
    let lab_end = newLab "or_end"
    let t2 = newReg "or_tmp2"
    let code2 = compileExp e2 vtable t2
    code1 @
        [ BNE (t1, Rzero, lab_true) ]
        @ code2 @
        [ LI (place, 0)
        ; J lab_end
        ; LABEL lab_true
